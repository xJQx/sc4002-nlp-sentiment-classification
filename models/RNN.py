...
class RNN(nn.Module):
    ...
    def forward(self, sequences, original_len):
        ...
        if self.sentence_representation_type == "last":
            sentence_representation = hidden[-1] if not self.bidirectional else torch.cat((hidden[0][-1], hidden[1][-1]), dim=1) # if bidirectional, concat the backward and forward hidden states
